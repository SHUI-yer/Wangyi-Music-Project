...
- **❤️ 跨页面点赞收藏系统**：实现了全局打通的“红心喜欢”系统。无论是播放条、首页、最近播放还是搜索页，一键红心，跨组件自动同步更新状态，并持久化到本地。
- **⚙️ 纯配置驱动 (Configuration-driven)**：将所有歌单分类、标题、文案介绍等硬编码从组件中彻底抽离。只需修改 `src/config/playlistConfig.js` 即可实现全站文案更新与分类动态扩容，极大提升了代码健壮性。
- **🧬 智能元数据识别**：
  - **文件名识别**：自动根据文件名（歌手 - 歌名）填充信息。
  - **ID3 深度解析**：支持从 MP3 文件内部提取 **标题、歌手、专辑、封面图**，解决中文乱码。
- **📁 工业级库管理**：
  - **本地库上传**：支持大文件（100MB+）串行稳定上传，永久存储至服务器。
  - **下载管理与批量操作**：引入现代化卡片式列表设计，支持全选、反选及批量删除。
  - **分类扫描**：自动识别 `rock`, `pop`, `chinese`, `jazz` 物理目录，实现“所见即所得”。
- **🎧 极致播放体验**：单曲循环、列表循环、随机播放完整逻辑，支持拖拽跳转与音量实时调节，修复了由于 HMR 导致的多实例死循环闪退问题。
...
- **架构与避坑指南 (必看)**：[PROJECT_DOCS.md](PROJECT_DOCS.md)
